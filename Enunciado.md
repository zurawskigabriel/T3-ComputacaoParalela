...
O objetivo do trabalho é implementar, usando a biblioteca MPI, uma versão paralela seguindo o modelo divisão e conquista, de um programa adequado para este modelo. Após implementado, o programa deve ser executado no cluster Atlantica com até 2 nós (variando de 1, 3, 7, 15 e 31 processos, este último com HT) para realização das medições de desempenho. Deve ser medida também uma versão onde o trabalho é balanceado na árvore. Cada relatório deve no formato .pdf e conter uma página com a análise dos resultados e o código nas páginas subsequentes (sem limitação de tamanho, fazer uma formatação em coluna única desta parte para facilitar análise).

Segue um exemplo abaixo do funcionamento do modelo na ordenação de um grande vetor. O nó raiz da árvore verifica se o vetor inicial será dividido ou conquistado. A verificação da condição de conquista é uma comparação com um valor fixo, para o vetor com 40 elementos pode ser por exemplo <= 10 (assim o algoritmo só desce 2 níveis da árvore binária e se utiliza dos 7 processadores alocados). Se for dividido, cada filho da árvore binária recebe uma metade do vetor. Por sua vez, cada filho verifica se ira conquistar ou novamente dividir o vetor. Quando as folhas da árvore decidirem que o vetor já está pequeno o bastante para ser conquistado, o vetor é ordenado com o algoritmo de Bubble Sort (código abaixo) e devolvido para o respectivo pai. O pai por sua vez, intercala os dois vetores recebidos (código sequencial abaixo) e devolve ao seu pai, até que o nó folha restabeleça o vetor original ordenado (Figura 1).

Para ordenar um vetor de 1.000.000 de elementos com um processo rodando em um núcleo na Atlantica o algoritmo de ordenação troca simples (bs) abaixo precisou de 74 minutos. Para 3 processos (um raiz e dois filhos) rodando em 3 núcleos precisou de 18 minutos.
...
Os itens para avaliação são:
    ...
    execução da versão sequencial do algoritmo escolhido;
    implementação da versão paralela SPMD do algoritmo MPI escolhido seguindo o modelo divisão e conquista;
    medição dos tempos de execução para a versão sequencial em uma máquina qualquer do aluno ou laboratório e da versão paralela (usando 2 nós exclusivos da máquina Atlantica totalizando 15 processadores - cada nó possui 8 processadores físicos);
    ...
    análise do ganho obtido com a inclusão de um mecanismo de balanceamento de carga;
    execute com mais processos (63, 127, 255, 511, 1023) e analise o desempenho obtido
...
Última atualização: quinta-feira, 18 jun. 2026, 18:18
